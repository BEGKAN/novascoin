...
const API_URL = 'http://192.168.1.100:3000'; // Узнайте ваш IP через ipconfig
...
function loadPage(page) {
    ...
    switch(page) {
        case 'home': content.innerHTML = window.pages.home.render(userData); window.pages.home.init(); break;
        case 'shop': content.innerHTML = window.pages.shop.render(userData); window.pages.shop.init(); break;
        case 'games': content.innerHTML = window.pages.games.render(); window.pages.games.init(); break;
        case 'rating': content.innerHTML = window.pages.rating.render(); window.pages.rating.init(); break;
        case 'profile': content.innerHTML = window.pages.profile.render(userData); window.pages.profile.init(); break;
    }
}
...
async function loadUserData(silent = false) {
    ...
    try {
        ...
        if (!silent) loadPage(document.querySelector('.nav-item.active').dataset.page);
    } catch (error) {
